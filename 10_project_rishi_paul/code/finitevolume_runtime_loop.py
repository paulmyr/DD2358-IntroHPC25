...
if __name__== "__main__":
    ...
    parser.add_argument("-n", "--n", help = "grid size (2**n x 2**n)")
    parser.add_argument("-t", "--total_iterations", help = "number of iterations total")

    _args = parser.parse_args()

    if not _args.total_iterations:
        _args.total_iterations = 256
    else:
        _args.total_iterations = int(_args.total_iterations)
    ...
    # main
    wtimes = main(2**_args.n, _args.total_iterations)
    X = np.arange(len(wtimes))
    plt.plot(X, wtimes, label=f"runtime per iteration on {2**_args.n}x{2**_args.n} grid", marker='o')

    # plt.xscale("log", base=2)
    plt.xlabel(f"iteration (total: {_args.total_iterations})")
    
    # plt.yscale("log", base=10)
    plt.ylabel(f"wtime (in s) of loop iteration")
    plt.legend()
