...
def test_rho(grid_size):
    ...
    rho_torch = runtimeplots_with_gpu.main(N=grid_size, tEnd=0.1, terminate_using="T")


    # Check that they are all close enough to the default implementation
    assert np.allclose(rho_default, rho_torch.cpu().detach().numpy())