# -*- coding: utf-8 -*-
"""runtimeplots_with_gpu.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Q4PelYR8o5Z87mp90nczX95mPLbbNKED
"""
...
def main(N=128, tEnd=2, plotRealTime=False):
    ...
    Mass = Mass.to(gpu, dtype=torch.float32)
    Momx = Momx.to(gpu, dtype=torch.float32)
    Momy = Momy.to(gpu, dtype=torch.float32)
    Energy = Energy.to(gpu, dtype=torch.float32)

    # Simulation Main Loop
    number_of_iterations = 0
    # while t < tEnd:
    while number_of_iterations < tEnd:
...
def run_function_as_experiment(f, lbound, ubound, num_runs, tEnd):
    ...
    for i, grid_size in enumerate(grid_sizes):
        ...
        wtimes[i] /= num_runs
        print(f"{lbound + i} done, took {wtimes[i]}s")
    ...
    plt.plot(grid_sizes, wtimes, label=f"Apple M1 torch M1GPU accelerated", marker='o')
...
if __name__== "__main__":
    run_function_as_experiment(main, 9, 12+1, 3, 256)
